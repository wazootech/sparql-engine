...
### `@wazoo/sparql-engine/sqlite` — durable SQLite store (64.3 KiB)

The server-only durable store over `node:sqlite` — the one subpath that pulls a
Node/Deno builtin. Use it for restart-safe SPARQL updates (crash-recovery suite
included; see `docs/durable-transactions.md`):
...
through `./term`, `./store`, `./sqlite`, `./parser`, or `./serialize` (or the
root entrypoint) is private surface.
...
sources (`*.jison`, `generate-parser.ts`, ~87 KiB); the server-only
`sqlite-store.ts` ships behind the `./sqlite` subpath rather than in the root
import graph. The chart and the `bench:size` JSON both mirror that file set
exactly.
