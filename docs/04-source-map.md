...
| `src/wazoo-sparql-engine.ts`     | `WazooSparqlTransaction` L16, `WazooSparqlEngineOptions` L33, `WazooSparqlEngine` L56                                                                                                             | Orchestrator: parse → dispatch query/update. Implements `SparqlEngineInterface`, drop-in for `ComunicaSparqlEngine`. |
| `src/quad-store.ts`              | `QuadIndex` L11, `buildQuadIndex` L21, `probeQuadIndex` L56, `storeVersion` L91, `matchQuads` L102, `simplePredicate` L133, `GraphScopedStore` L147, `namedGraphs` L176, `buildDatasetStore` L199 | The store-adapter layer: stream draining, positional indexes, graph scoping, active-dataset materialization.         |
...
| `bgp-evaluator.ts`         | `ExistsSnapshot` L54 (internal), `BgpEvaluatorOptions` L63, `BgpEvaluator` L89, `patternListContainsExists` L1044                                                                                                                                                                                                                                                                                                                  | Pattern algebra: group evaluation, OPTIONAL/MINUS/UNION/GRAPH/BIND/VALUES, subqueries, and the synchronous EXISTS evaluator (per-call snapshot, issue #72). |
| `join.ts`                  | `TermBinding` L21, `ScanEntry` L28, `BindingFilter` L52, `bindingsCompatible` L59, `leftJoin` L82, `innerJoin` L154, `minus` L202, `scanEntry` L454, `joinTriplePattern` L520, `PathPair` L767, `PathEntry` L776, `PathElement` L785, `isPropertyPath` L791, `isMultisetPath` L807, `scanPathEntry` L828, `joinPathPattern` L880, `matchQuadsSync` L1252, `graphNodesSync` L1270, `pathStepsSync` L1289, `scanPathEntrySync` L1550 | The join engine: hash joins, binding-set algebra, property paths, plus the synchronous twins used by EXISTS.                                                |
...
| File                   | Symbols (line)                                        | Role                                                                             |
| ---------------------- | ----------------------------------------------------- | -------------------------------------------------------------------------------- |
| `memory-store.ts`      | `Listener` L4, `MemoryStream` L36, `MemoryStore` L273 | Zero-dependency in-memory store + RDF/JS stream implementation.                  |
